  ...
  <div class="grid flex align-items-center mt-3 mb-5">
    ...
    <div class="col-12 md:col-6 md:text-right">
      <Button
        ...
        @click="open"
        v-if="AnimalSecretStatus.includes(9) && permit[0].IsAdd"
      />
    </div>
  </div>
  <DataTable
    ...
    <form class="grid mt-2">
      <div class="col-12 lg:col-12">
        <div class="grid">
          ...
          <div class="col-12 lg:col-6">
            <label class="block text-600 text-sm font-bold mb-2">
              วันที่หย่านม<span class="text-red-500"> *</span></label
            >
            <!-- <Calendar
              :manualInput="false"
              :maxDate="new Date()"
              v-model="data[index].WeanMilkDate"
              class="w-full"
              :class="{
                'p-invalid': !data[index].WeanMilkDate && valid,
              }"
            /> -->

            <Datepicker
              v-model="data[index].WeanMilkDate"
              :class="{ 'p-invalid': !data[index].WeanMilkDate && valid }"
              id="dateRange"
              locale="th"
              :format="format"
              utc
...
export default {
  ...
  async mounted() {
    ...
    if (
      this.display_prop &&
      this.AnimalSecretStatus.includes(9) &&
      this.permit[0].IsAdd
    ) {
      this.open();
    }
  ...
  methods: {
    ...
    clear() {
      ...
      this.index = null;
      this.valid = false;
      this.$emit("clear_display");
    },
    // validate templates
    validation() {
      if (
        !this.data[this.index].ResponsibilityStaffID ||
        !this.data[this.index].WeanMilkDate
      ) {
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
