...
export default {
  ...
  mounted() {
    ...
    axios
      ...
      .then((response) => {
        this.EmbryoAll = response.data.rows.map((item) => {
          return {
  ...
  methods: {
    loadData() {
      ...
      const setUrl = (this.apiTransferEmbryo += "/" + route.params.id);

      axios.get(setUrl, { signal: this.controller.signal }).then((response) => {
        // console.log(response.data.AnimalID);
        let id = response.data;

        this.form = {
          AnimalID: id.AnimalID,
          TransferEmbryoID: id.TransferEmbryoID,
          TransferDate: id.TransferDate,
          PAR: id.PAR,
          TimeNo: id.TimeNo,
          EmbryoNumber: id.EmbryoNumber,
          BCSID: id.BCSID,
          CLType: id.CLType,
          CLSize: id.CLSize,
          CavitySize: id.CavitySize,
          UterusSide: id.UterusSide,
          EmbryoID: id.EmbryoID,
          TransferMethodID: id.TransferMethodID,
          StandingHeatDate: id.StandingHeatDate,
          EstimateBirthDate: id.EstimateBirthDate,
          BirthDate: id.BirthDate,
          LeftOvaryAmount: id.LeftOvaryAmount,
          RightOvaryAmount: id.RightOvaryAmount,
          LeftOvaryGrade: id.LeftOvaryGrade,
          RightOvaryGrade: id.RightOvaryGrade,
    ...
    async add() {
      ...
      await axios
        .put("/transfer-embryo/" + this.id, this.form)
        .then(() => {
          // console.log(res);
