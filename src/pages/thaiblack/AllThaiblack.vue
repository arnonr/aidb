                        ...
                        >
                            ...
                            <Column header="จัดการ" style="width: 200px">
                                <template #body="slotProps">
                                    <SplitButton
                                        @click="
                                            edit(slotProps.data.ThaiblackID)
                                        "
                                        label="แก้ไข"
                                        icon="pi pi-pencil"
                                        class="p-button-sm p-button-outlined p-button-warning"
                                        :model="
                                            getItems(slotProps.data.ThaiblackID)
                                        "
                                    >
                                    </SplitButton>
                                </template>
...
export default {
    ...
    methods: {
        ...
        async update() {
            ...
            axios
                .put("thaiblack/" + this.form.ThaiblackID, this.form)
                .then(() => {
                    this.close_edit();
                    this.load();
                    this.$toast.add({
                    ...
                    this.data = res.data.rows
                        .map((x) => {
                            x.Round = this.project_level.find((e) => {
                                return e.id == x.ThaiblackRound;
                            }).name;
                            return x;
                        })
                        .sort(
                            (a, b) =>
                                new Date(b.ThaiblackDate) -
                                new Date(a.ThaiblackDate)
                        );
        ...
        // remove data
        remove() {
            axios.delete("thaiblack/" + this.data.id).then(() => {
                this.close_delete();
                this.load();
                this.$toast.add({
                    severity: "success",
