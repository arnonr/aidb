<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สาขาวิชา" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สาขาวิชา</h1>
          </div>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.MajorID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.MajorID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสสาขาวิชา</label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.MajorCode && valid }"
                v-model="form.MajorCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> ชื่อสาขาวิชา </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.MajorName"
                :class="{ 'p-invalid': !form.MajorName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.MajorCode || !this.form.MajorName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.MajorID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.MajorID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        MajorID: id.MajorID,
        MajorCode: id.MajorCode,
        MajorName: id.MajorName,
        MajorNameEN: id.MajorNameEN,
        show_id: id.show_id,
