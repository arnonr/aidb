<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ข้อมูลศูนย์วิจัย" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ข้อมูลศูนย์วิจัย</h1>
          </div>
        ...
        >
          ...
          <Column
            field="AIZoneCode"
            header="รหัสศูนย์วิจัย"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="AIZoneName"
            header="ชื่อศูนย์วิจัย"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="AIZoneENCode"
            header="รหัสศูนย์วิจัยตัวย่อภาษาอังกฤษ"
            class="text-center"
            :sortable="true"
          >
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.AIZoneID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.AIZoneID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-600 text-sm font-bold mb-2">
                รหัสศูนย์วิจัย</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.AIZoneCode && valid }"
                v-model="form.AIZoneCode"
              />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-600 text-sm font-bold mb-2">
              ...
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.AIZoneName && valid }"
                v-model="form.AIZoneName"
              />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-600 text-sm font-bold mb-2">
                รหัสศูนย์วิจัยตัวย่อภาษาอังกฤษ
              </label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.AIZoneENCode && valid }"
                v-model="form.AIZoneENCode"
              />
            </div>
            <div class="field col-12 sm:col-12 lg:col-12">
              ...
              >
                ...
                />
                <label :for="isActive.key">{{ isActive.AIZoneName }}</label>
              </div>
            </div>
          </div>
        </div>
    ...
    >
      <div class="confirmation-content text-center">
        ...
        <br />
        <span class="text-xl"
          >คุณต้องการลบข้อมูลศูนย์วิจัยลำดับที่
...
export default {
  ...
  data() {
    return {
      ...
      status: [
        { AIZoneName: "เปิดการใช้งาน", key: "A", value: 1 },
        { AIZoneName: "ปิดการใช้งาน", key: "B", value: 0 },
      ],
  ...
  methods: {
    ...
    validation() {
      if (!this.form.AIZoneCode || !this.form.AIZoneName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    open_edit(id) {
      id = this.data.find((x) => x.AIZoneID == id);
      // console.log(id);
      if (id.isActive == 0) {
      ...
      this.form = {
        id: id.AIZoneID,
        AIZoneCode: id.AIZoneCode,
        show_id: id.show_id,
        AIZoneName: id.AIZoneName,
        AIZoneENCode: id.AIZoneENCode,
