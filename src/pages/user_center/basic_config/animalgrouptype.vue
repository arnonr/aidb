<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ชนิดสัตว์หลัก" :pages="breadcrumb" />
        ...
        <div class="mt-5 card">
          <div class="grid flex align-items-center mb-5">
            <div class="col-12 md:col-6">
              <h1 class="text-2xl mb-0 text-600">ชนิดสัตว์หลัก</h1>
            </div>
          ...
          <DataTable class="text-sm" :value="data" :paginator="true" :rows="10" v-model:filters="search"
            ...
            <Column field="AnimalGroupTypeCode" header="รหัสชนิดสัตว์หลัก" class="text-center" :sortable="true">
            </Column>
            <Column field="AnimalGroupTypeName" header="ชื่อชนิดสัตว์หลัก" class="text-center" :sortable="true">
            </Column>
            <Column field="AnimalGenre.AnimalGenreName" header="ชื่อประเภทสัตว์" class="text-center" :sortable="true">
            </Column>
            ...
            <Column header="จัดการ" class="text-center">
              <template #body="slotProps">
                <Button label="แก้ไข" icon="pi pi-pencil" class="p-button-warning text-center mr-1"
                  @click="open_edit(slotProps.data.AnimalGroupTypeID)" />
                <Button label="ลบ" icon="pi pi-trash" class="p-button-danger text-center ml-1" @click="
                  open_delete(
                    slotProps.data.AnimalGroupTypeID,
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสชนิดสัตว์หลัก</label>
              <InputText type="text" class="w-full" :class="{ 'p-invalid': !form.AnimalGroupTypeCode && valid }"
                v-model="form.AnimalGroupTypeCode" />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText type="text" class="w-full" v-model="form.AnimalGroupTypeName"
                :class="{ 'p-invalid': !form.AnimalGroupTypeName && valid }" />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">ประเภทสัตว์</label>
              <Dropdown class="w-full" placeholder="เลือกประเภทสัตว์" optionLabel="AnimalGenreName"
                optionValue="AnimalGenreID" v-model="form.AnimalGenreID"
                :class="{ 'p-invalid': !form.AnimalGenreID && valid }" :options="genre" :filter="true" />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/animal-group-type",
      getGenre: "/animal-genre",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "ชนิดสัตว์หลัก", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.AnimalGroupTypeCode || !this.form.AnimalGroupTypeName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    load() {
      ...
      axios.get(this.getGenre).then((response) => {
        this.genre = response.data.rows;
      });
    },
    add() {
      // validation
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.AnimalGroupTypeID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.AnimalGroupTypeID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        ...
        AnimalGroupTypeCode: id.AnimalGroupTypeCode,
        AnimalGroupTypeName: id.AnimalGroupTypeName,
