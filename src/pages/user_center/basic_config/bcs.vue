<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="คะแนนร่างกาย" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">คะแนนร่างกาย</h1>
          </div>
        ...
        >
          ...
          <Column
            field="BCSNumber"
            header="รหัสคะแนนร่างกาย"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="BCSName"
            header="ชื่อคะแนนร่างกาย"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.BCSID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.BCSID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสคะแนนร่างกาย</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.BCSNumber && valid }"
                v-model="form.BCSNumber"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.BCSName"
                :class="{ 'p-invalid': !form.BCSName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.BCSNumber || !this.form.BCSName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.BCSID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.BCSID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        BCSID: id.BCSID,
        BCSNumber: id.BCSNumber,
        BCSName: id.BCSName,
        BCSNameEN: id.BCSNameEN,
        show_id: id.show_id,
