<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สาเหตุโน้มนำด้านสิ่งแวดล้อม" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สาเหตุโน้มนำด้านสิ่งแวดล้อม</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="CauseEnvironmentCode"
            header="รหัสสาเหตุโน้มนำด้านสิ่งแวดล้อม"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="CauseEnvironmentName"
            header="ชื่อสาเหตุโน้มนำด้านสิ่งแวดล้อม"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.CauseEnvironmentID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.CauseEnvironmentID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสสาเหตุโน้มนำด้านสิ่งแวดล้อม</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.CauseEnvironmentCode && valid }"
                v-model="form.CauseEnvironmentCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อสาเหตุโน้มนำด้านสิ่งแวดล้อม
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.CauseEnvironmentName"
                :class="{ 'p-invalid': !form.CauseEnvironmentName && valid }"
              />
            </div>
            <div class="field col-12 sm:col-12">
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.CauseEnvironmentCode || !this.form.CauseEnvironmentName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.CauseEnvironmentID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.CauseEnvironmentID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        CauseEnvironmentID: id.CauseEnvironmentID,
        CauseEnvironmentCode: id.CauseEnvironmentCode,
        CauseEnvironmentName: id.CauseEnvironmentName,
