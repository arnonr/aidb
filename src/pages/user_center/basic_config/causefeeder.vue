<template>
  <div class="grid">
    ...
    <div class="col-12">
      <PageTitle
        title="สาเหตุโน้มนำการเลี้ยงและการจัดการ"
        :pages="breadcrumb"
      />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">
              สาเหตุโน้มนำการเลี้ยงและการจัดการ
            </h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="CauseFeederCode"
            header="รหัสสาเหตุโน้มนำการเลี้ยงและการจัดการ"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="CauseFeederName"
            header="ชื่อสาเหตุโน้มนำการเลี้ยงและการจัดการ"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.CauseFeederID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสสาเหตุโน้มนำการเลี้ยงและการจัดการ</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.CauseFeederCode && valid }"
                v-model="form.CauseFeederCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อสาเหตุโน้มนำการเลี้ยงและการจัดการ
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.CauseFeederName"
                :class="{ 'p-invalid': !form.CauseFeederName && valid }"
              />
            </div>
            <div class="field col-12 sm:col-12">
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumb: [
        ...
        { label: "สาเหตุโน้มนำการเลี้ยงและการจัดการ", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.CauseFeederCode || !this.form.CauseFeederName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.CauseFeederID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.CauseFeederID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        CauseFeederID: id.CauseFeederID,
        CauseFeederCode: id.CauseFeederCode,
        CauseFeederName: id.CauseFeederName,
