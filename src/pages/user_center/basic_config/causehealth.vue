<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สาเหตุโน้มนำด้านสุขภาพ" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สาเหตุโน้มนำด้านสุขภาพ</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="CauseHealthCode"
            header="รหัสสาเหตุโน้มนำด้านสุขภาพ"
            class="text-center"
            :sortable="true"
          >
          </Column>
          <Column
            field="CauseHealthName"
            header="ชื่อสาเหตุโน้มนำด้านสุขภาพ"
            class="text-center"
            :sortable="true"
          >
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.CauseHealthID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสสาเหตุโน้มนำด้านสุขภาพ</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.CauseHealthCode && valid }"
                v-model="form.CauseHealthCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อสาเหตุโน้มนำด้านสุขภาพ
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.CauseHealthName"
                :class="{ 'p-invalid': !form.CauseHealthName && valid }"
              />
            </div>
            <div class="field col-12 sm:col-12">
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumb: [
        ...
        { label: "สาเหตุโน้มนำด้านสุขภาพ", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.CauseHealthCode || !this.form.CauseHealthName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.CauseHealthID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.CauseHealthID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        CauseHealthID: id.CauseHealthID,
        CauseHealthCode: id.CauseHealthCode,
        CauseHealthName: id.CauseHealthName,
