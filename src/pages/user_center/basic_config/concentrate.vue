<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="อาหารข้น" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">อาหารข้น</h1>
          </div>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.FoodID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.FoodID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> ชื่ออาหารข้น </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.FoodName"
                :class="{ 'p-invalid': !form.FoodName && valid }"
              />
            </div>
  ...
  methods: {
    ...
    load() {
      axios.get(this.url).then((response) => {
        this.data = response.data.rows.filter((item) => item.FoodType == "1");
    ...
    update() {
      ...
      axios
        .put(this.url + "/" + this.form.FoodID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.FoodID == id);
      if (id.isActive == 0) {
