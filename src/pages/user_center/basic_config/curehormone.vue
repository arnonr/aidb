<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ฮอร์โมน" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ฮอร์โมน</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="CureHormoneCode"
            header="รหัสฮอร์โมน"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="CureHormoneName"
            header="ชื่อฮอร์โมน"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.CureHormoneID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสฮอร์โมน</label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.CureHormoneCode && valid }"
                v-model="form.CureHormoneCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> ชื่อฮอร์โมน </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.CureHormoneName"
                :class="{ 'p-invalid': !form.CureHormoneName && valid }"
              />
            </div>
            <div class="field col-12 sm:col-12">
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.CureHormoneCode || !this.form.CureHormoneName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.CureHormoneID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.CureHormoneID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        CureHormoneID: id.CureHormoneID,
        CureHormoneCode: id.CureHormoneCode,
        CureHormoneName: id.CureHormoneName,
