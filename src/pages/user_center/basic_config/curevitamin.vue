<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ยาและวิตามิน" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ยาและวิตามิน</h1>
          </div>
        ...
        <DataTable class="text-sm" :value="data" :paginator="true" :rows="10" v-model:filters="search"
          ...
          <Column field="CureVitaminCode" header="รหัสยาและวิตามิน" class="text-center" :sortable="true"></Column>
          <Column field="CureVitaminName" header="ชื่อยาและวิตามิน" class="text-center" :sortable="true"></Column>
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสยาและวิตามิน</label>
              <InputText type="text" class="w-full" :class="{ 'p-invalid': !form.CureVitaminCode && valid }"
                v-model="form.CureVitaminCode" />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText type="text" class="w-full" v-model="form.CureVitaminName"
                :class="{ 'p-invalid': !form.CureVitaminName && valid }" />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-600 text-sm font-bold mb-2">ชนิดสัตว์</label>

              <MultiSelect class="w-full" :options="animaltype" optionLabel="AnimalTypeName" optionValue="AnimalTypeID"
                placeholder="เลือกชนิดสัตว์" display="chip" v-model="form.AnimalTypeID"
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.CureVitaminCode || !this.form.CureVitaminName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.CureVitaminID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.CureVitaminID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        CureVitaminID: id.CureVitaminID,
        CureVitaminCode: id.CureVitaminCode,
        CureVitaminName: id.CureVitaminName,
