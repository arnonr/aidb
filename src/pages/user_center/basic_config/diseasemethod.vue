<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="วิธีการตรวจโรค" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">วิธีการตรวจโรค</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="DiseaseMethodCode"
            header="รหัสวิธีการตรวจโรค"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="DiseaseMethodName"
            header="ชื่อวิธีการตรวจโรค"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.DiseaseMethodID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.DiseaseMethodID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสวิธีการตรวจโรค</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.DiseaseMethodCode && valid }"
                v-model="form.DiseaseMethodCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.DiseaseMethodName"
                :class="{ 'p-invalid': !form.DiseaseMethodName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.DiseaseMethodCode || !this.form.DiseaseMethodName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.DiseaseMethodID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.DiseaseMethodID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        DiseaseMethodID: id.DiseaseMethodID,
        DiseaseMethodCode: id.DiseaseMethodCode,
        DiseaseMethodName: id.DiseaseMethodName,
        DiseaseMethodNameEN: id.DiseaseMethodNameEN,
        show_id: id.show_id,
