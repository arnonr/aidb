<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ผลการตรวจโรค" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ผลการตรวจโรค</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="DiseaseResultCode"
            header="รหัสผลการตรวจโรค"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="DiseaseResultName"
            header="ชื่อผลการตรวจโรค"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.DiseaseResultID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.DiseaseResultID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสผลการตรวจโรค</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.DiseaseResultCode && valid }"
                v-model="form.DiseaseResultCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.DiseaseResultName"
                :class="{ 'p-invalid': !form.DiseaseResultName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.DiseaseResultCode || !this.form.DiseaseResultName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.DiseaseResultID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.DiseaseResultID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        DiseaseResultID: id.DiseaseResultID,
        DiseaseResultCode: id.DiseaseResultCode,
        DiseaseResultName: id.DiseaseResultName,
        DiseaseResultNameEN: id.DiseaseResultNameEN,
        show_id: id.show_id,
