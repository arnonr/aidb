<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สาเหตุการคัดจำหน่าย" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สาเหตุการคัดจำหน่าย</h1>
          </div>
        ...
        >
          ...
          <Column
            ...
            header="ชื่อสาเหตุการคัดจำหน่าย"
            class="text-center"
            :sortable="true"
          >
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.DistributionReasonID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.DistributionReasonID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อสาเหตุการคัดจำหน่าย
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.DistributionReasonName"
                :class="{ 'p-invalid': !form.DistributionReasonName && valid }"
              />
            </div>
            <div class="field col-12 sm:col-12">
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.AnimalTypeID || !this.form.DistributionReasonName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.DistributionReasonID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.DistributionReasonID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        DistributionReasonID: id.DistributionReasonID,
        AnimalTypeID: id.AnimalTypeID,
        DistributionReasonName: id.DistributionReasonName,
        show_id: id.show_id,
