<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ระยะตัวอ่อน" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ระยะตัวอ่อน</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="EmbryoStageCode"
            header="รหัสระยะตัวอ่อน"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="EmbryoStageName"
            header="ชื่อระยะตัวอ่อน"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
          ...
          <Column field="IsTransfer" header="ย้ายฝากสถานะ" class="text-center">
            <template #body="slotProps">
              <div
                v-if="
                  slotProps.data.IsTransfer == 1 ||
                  slotProps.data.IsTransfer == status[0]
                "
              >
                <Tag class="w-full" severity="success">สามารถย้ายฝาก</Tag>
              </div>
              <div v-else>
                <Tag class="w-full bg-gray-500">ไม่สามารถย้ายฝาก</Tag>
              </div>
            </template>
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.EmbryoStageID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสระยะตัวอ่อน</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{
                  'p-invalid': !form.EmbryoStageCode && valid,
                }"
                v-model="form.EmbryoStageCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.EmbryoStageName"
                :class="{
                  'p-invalid': !form.EmbryoStageName && valid,
                }"
              />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> เกรด </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.EmbryoStageGrade"
                :class="{
                  'p-invalid': !form.EmbryoStageGrade && valid,
                }"
              />
            </div>
            <div class="field col-12 sm:col-12 md:col-12">
              <label class="block text-600 text-sm font-bold mb-2">
                สถานะการย้ายฝาก</label
              >
              <Dropdown
                class="w-full"
                :options="extend"
                optionLabel="name"
                placeholder="เลือกสถานะการย้ายฝาก"
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (
        !this.form.EmbryoStageCode ||
        !this.form.EmbryoStageName ||
    ...
    add() {
      ...
      this.form.IsTransfer = this.form.IsTransfer.id;
      // post
      axios
    ...
    update() {
      ...
      this.form.IsTransfer = this.form.IsTransfer.id;
      axios
        .put(this.url + "/" + this.form.EmbryoStageID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.EmbryoStageID == id);
      if (id.isActive == 0) {
      ...
      if (id.IsTransfer == 0) {
        id.IsTransfer = this.extend[1];
      } else if (id.IsTransfer == 1) {
        id.IsTransfer = this.extend[0];
      }

      this.form = {
        EmbryoStageID: id.EmbryoStageID,
        EmbryoStageCode: id.EmbryoStageCode,
        EmbryoStageName: id.EmbryoStageName,
        EmbryoStageGrade: id.EmbryoStageGrade,
        IsTransfer: id.IsTransfer,
        show_id: id.show_id,
