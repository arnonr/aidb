<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สถานะฟาร์ม" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สถานะฟาร์ม</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="FarmStatusCode"
            header="รหัสสถานะฟาร์ม"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="FarmStatusName"
            header="ชื่อสถานะฟาร์ม"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.FarmStatusID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสสถานะฟาร์ม</label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.FarmStatusCode && valid }"
                v-model="form.FarmStatusCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.FarmStatusName"
                :class="{ 'p-invalid': !form.FarmStatusName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.FarmStatusCode || !this.form.FarmStatusName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.FarmStatusID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.FarmStatusID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        FarmStatusID: id.FarmStatusID,
        FarmStatusCode: id.FarmStatusCode,
        FarmStatusName: id.FarmStatusName,
        FarmStatusNameEN: id.FarmStatusNameEN,
        show_id: id.show_id,
