<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ปืนสอดลึก" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ปืนสอดลึก</h1>
          </div>
        ...
        >
          ...
          <Column
            field="GunDepthCode"
            header="รหัสปืนสอดลึก"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="GunDepthName"
            header="ชื่อปืนสอดลึก"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.GunDepthID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.GunDepthID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสปืนสอดลึก</label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.GunDepthCode && valid }"
                v-model="form.GunDepthCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.GunDepthName"
                :class="{ 'p-invalid': !form.GunDepthName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.GunDepthCode || !this.form.GunDepthName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.GunDepthID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.GunDepthID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        GunDepthID: id.GunDepthID,
        GunDepthCode: id.GunDepthCode,
        GunDepthName: id.GunDepthName,
        GunDepthNameEN: id.GunDepthNameEN,
        show_id: id.show_id,
