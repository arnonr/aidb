<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="วงรอบการเป็ดสัด" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">วงรอบการเป็ดสัด</h1>
          </div>
        ...
        >
          ...
          <Column
            field="HeatCircleName"
            header="ชื่อวงรอบการเป็ดสัด"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.HeatCircleID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.HeatCircleName"
                :class="{ 'p-invalid': !form.HeatCircleName && valid }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/heat-circle",
      apiHeatCircleID: "/animal-type?isActive=1",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "วงรอบการเป็ดสัด", to: "" },
      ],
      search: {
  ...
  mounted() {
    ...
    axios.get(this.apiHeatCircleID).then((response) => {
      this.HeatCircle = response.data.rows;
    });
  },
  methods: {
    clear() {
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.HeatCircleID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.HeatCircleID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        HeatCircleID: id.HeatCircleID,

        HeatCircleName: id.HeatCircleName,
        show_id: id.show_id,
