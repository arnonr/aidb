<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="อาการเป็นสัด" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">อาการเป็นสัด</h1>
          </div>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.HeatTypeID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.HeatTypeID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.HeatTypeName"
                :class="{ 'p-invalid': !form.HeatTypeName && valid }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/heat-type",
      apiHeatTypeID: "/animal-type?isActive=1",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "อาการเป็นสัด", to: "" },
      ],
      search: {
  ...
  mounted() {
    ...
    axios.get(this.apiHeatTypeID).then((response) => {
      this.HeatType = response.data.rows;
    });
  },
  methods: {
    clear() {
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.HeatTypeID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.HeatTypeID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        HeatTypeID: id.HeatTypeID,

        HeatTypeName: id.HeatTypeName,
        show_id: id.show_id,
