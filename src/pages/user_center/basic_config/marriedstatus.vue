<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สถานภาพการสมรส" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สถานภาพการสมรส</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="MarriedStatusCode"
            header="รหัสสถานภาพการสมรส"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="MarriedStatusName"
            header="ชื่อสถานภาพการสมรส"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.MarriedStatusID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.MarriedStatusID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสสถานภาพการสมรส</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.MarriedStatusCode && valid }"
                v-model="form.MarriedStatusCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.MarriedStatusName"
                :class="{ 'p-invalid': !form.MarriedStatusName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.MarriedStatusCode || !this.form.MarriedStatusName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.MarriedStatusID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.MarriedStatusID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        MarriedStatusID: id.MarriedStatusID,
        MarriedStatusCode: id.MarriedStatusCode,
        MarriedStatusName: id.MarriedStatusName,
        MarriedStatusNameEN: id.MarriedStatusNameEN,
        show_id: id.show_id,
