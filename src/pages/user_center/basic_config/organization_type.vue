<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ข้อมูลประเภทหน่วยงาน" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ข้อมูลประเภทหน่วยงาน</h1>
          </div>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.OrganizationTypeID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.OrganizationTypeID,
    ...
    <!-- Add Dialog -->
    <Dialog
      header="เพิ่มข้อมูลประเภทหน่วยงาน"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-600 text-sm font-bold mb-2">
                รหัสประเภทหน่วยงาน</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.OrganizationTypeCode && valid }"
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.OrganizationTypeName && valid }"
                v-model="form.OrganizationTypeName"
              />
            </div>
            <div class="field col-12 sm:col-12 lg:col-12">
              ...
              >
                <RadioButton
                  :id="isActive.key"
                  OrganizationTypeName="category"
                  :value="isActive"
                  v-model="form.isActive"
                />
                <label :for="isActive.key">{{
                  isActive.OrganizationTypeName
                }}</label>
              </div>
            </div>
          </div>
    ...
    <!-- EDIT Dialog -->
    <Dialog
      header="แก้ไขข้อมูลประเภทหน่วยงาน"
    ...
    >
      <div class="confirmation-content text-center">
        ...
        <br />
        <span class="text-xl"
          >คุณต้องการลบข้อมูลประเภทหน่วยงานลำดับ
...
export default {
  ...
  data() {
    return {
      ...
      status: [
        { OrganizationTypeName: "เปิดการใช้งาน", key: "A", value: 1 },
        { OrganizationTypeName: "ปิดการใช้งาน", key: "B", value: 0 },
      ],
  ...
  methods: {
    ...
    validation() {
      if (!this.form.OrganizationTypeCode || !this.form.OrganizationTypeName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    open_edit(id) {
      id = this.data.find((x) => x.OrganizationTypeID == id);
      // console.log(id);
      if (id.isActive == 0) {
      ...
      this.form = {
        id: id.OrganizationTypeID,
        OrganizationTypeCode: id.OrganizationTypeCode,
        show_id: id.show_id,
        OrganizationTypeName: id.OrganizationTypeName,
    ...
    open_delete(id, show_id) {
      ...
      id = this.data.find((x) => x.OrganizationTypeID == id);
      this.form.id = id.OrganizationTypeID;
