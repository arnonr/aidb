<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ปัญหาอื่นๆ" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ปัญหาอื่น ๆ</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="OtherSymptomCode"
            header="รหัสปัญหาอื่นๆ"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="OtherSymptomName"
            header="ชื่อปัญหาอื่นๆ"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.OtherSymptomID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสปัญหาอื่น ๆ</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.OtherSymptomCode && valid }"
                v-model="form.OtherSymptomCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.OtherSymptomName"
                :class="{ 'p-invalid': !form.OtherSymptomName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.OtherSymptomCode || !this.form.OtherSymptomName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.OtherSymptomID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.OtherSymptomID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        OtherSymptomID: id.OtherSymptomID,
        OtherSymptomCode: id.OtherSymptomCode,
        OtherSymptomName: id.OtherSymptomName,
        OtherSymptomNameEN: id.OtherSymptomNameEN,
        show_id: id.show_id,
