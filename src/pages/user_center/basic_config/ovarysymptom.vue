<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ปัญหารังไข่" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ปัญหารังไข่</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="OvarySymptomCode"
            header="รหัสปัญหารังไข่"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="OvarySymptomName"
            header="ชื่อปัญหารังไข่"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.OvarySymptomID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสปัญหารังไข่</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.OvarySymptomCode && valid }"
                v-model="form.OvarySymptomCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.OvarySymptomName"
                :class="{ 'p-invalid': !form.OvarySymptomName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.OvarySymptomCode || !this.form.OvarySymptomName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.OvarySymptomID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.OvarySymptomID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        OvarySymptomID: id.OvarySymptomID,
        OvarySymptomCode: id.OvarySymptomCode,
        OvarySymptomName: id.OvarySymptomName,
        OvarySymptomNameEN: id.OvarySymptomNameEN,
        show_id: id.show_id,
