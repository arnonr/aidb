<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="วิธีการตรวจการตั้งท้อง" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">วิธีการตรวจการตั้งท้อง</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="PregnancyCheckMethodCode"
            header="รหัสวิธีการตรวจการตั้งท้อง"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="PregnancyCheckMethodName"
            header="ชื่อวิธีการตรวจการตั้งท้อง"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.AbortResultID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสวิธีการตรวจการตั้งท้อง</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{
                  'p-invalid': !form.PregnancyCheckMethodCode && valid,
                }"
                v-model="form.PregnancyCheckMethodCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อวิธีการตรวจการตั้งท้อง
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.PregnancyCheckMethodName"
                :class="{
                  'p-invalid': !form.PregnancyCheckMethodName && valid,
                }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/pregnancy-check-method",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "วิธีการตรวจการตั้งท้อง", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (
        !this.form.PregnancyCheckMethodCode ||
        !this.form.PregnancyCheckMethodName
      ) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.AbortResultID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.AbortResultID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        AbortResultID: id.AbortResultID,
        PregnancyCheckMethodCode: id.PregnancyCheckMethodCode,
        PregnancyCheckMethodName: id.PregnancyCheckMethodName,
        PregnancyCheckMethodNameEN: id.PregnancyCheckMethodNameEN,
        show_id: id.show_id,
