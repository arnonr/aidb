<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สถานะตรวจการตั้งท้อง" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สถานะตรวจการตั้งท้อง</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="PregnancyCheckStatusCode"
            header="รหัสสถานะตรวจการตั้งท้อง"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="PregnancyCheckStatusName"
            header="ชื่อสถานะตรวจการตั้งท้อง"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.PregnancyCheckStatusID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.PregnancyCheckStatusID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสสถานะตรวจการตั้งท้อง</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{
                  'p-invalid': !form.PregnancyCheckStatusCode && valid,
                }"
                v-model="form.PregnancyCheckStatusCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อสถานะตรวจการตั้งท้อง
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.PregnancyCheckStatusName"
                :class="{
                  'p-invalid': !form.PregnancyCheckStatusName && valid,
                }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/pregnancy-check-status",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "สถานะตรวจการตั้งท้อง", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (
        !this.form.PregnancyCheckStatusCode ||
        !this.form.PregnancyCheckStatusName
      ) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.PregnancyCheckStatusID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.PregnancyCheckStatusID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        PregnancyCheckStatusID: id.PregnancyCheckStatusID,
        PregnancyCheckStatusCode: id.PregnancyCheckStatusCode,
        PregnancyCheckStatusName: id.PregnancyCheckStatusName,
        PregnancyCheckStatusNameEN: id.PregnancyCheckStatusNameEN,
        show_id: id.show_id,
