<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="สถานภาพการผลิต" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">สถานภาพการผลิต</h1>
          </div>
          <div class="col-12 md:col-6 md:text-right">
            <Button @click="open_add" label="เพิ่มข้อมูล" icon="pi pi-plus" class="p-button-primary" />
          </div>
        </div>
        <DataTable class="text-sm" :value="data" :loading="isLoading" :paginator="true" :rows="10"
          ...
          <Column field="code" header="รหัส" class="text-center" :sortable="true"></Column>
          <Column field="name" header="สถานภาพการผลิต" class="text-center"></Column>
          <Column field="is_active" header="สถานะ" class="text-center" :sortable="true">
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              ...
              <Button label="ลบ" icon="pi pi-trash" class="p-button-danger text-center ml-1" @click="
                open_delete(slotProps.data?.id, slotProps.data?.show_id)
              " />
            </template>
          </Column>
        </DataTable>
    ...
    <!-- end main page -->
    <!-- Add Dialog -->
    <Dialog header="เพิ่มสถานะสัตว์" v-model:visible="display_add" :style="{ width: '50vw' }" :modal="true">
      <form class="grid mt-2">
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัส</label>
              <InputText type="text" class="w-full" :class="{ 'p-invalid': !form.code && valid }" v-model="form.code" />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                สถานภาพการผลิต
              </label>
              <InputText type="text" class="w-full" v-model="form.name" :class="{ 'p-invalid': !form.name && valid }" />
            </div>
            <div class="field col-12 sm:col-12 lg:col-12">
              <div class="text-xl">สถานะ</div>
              <div v-for="is_active of status" :key="is_active.key" class="field-radiobutton">
                <RadioButton :id="is_active.key" name="category" :value="is_active" v-model="form.is_active" />
    ...
    <!-- End Add Dialog -->
    <!-- EDIT Dialog -->
    <Dialog header="แก้ไขสถานะสัตว์" v-model:visible="display_edit" :style="{ width: '50vw' }" :modal="true">
      <form class="grid mt-2">
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
            ...
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> สถานภาพการผลิต </label>
              <InputText type="text" class="w-full" v-model="form.name" :class="{ 'p-invalid': !form.name && valid }" />
            </div>
            <div class="field col-12 sm:col-12 lg:col-12">
              <div class="text-xl">สถานะ</div>
    ...
    <Dialog header="ตรวจสอบข้อมูล" v-model:visible="display_delete" :style="{ width: '350px' }" :modal="true">
      <div class="confirmation-content text-center">
        ...
        <br />
        <span class="text-xl">คุณต้องการลบสถานะสัตว์ลำดับที่ {{ form.show_id }} ใช่หรือไม่</span>
      </div>
      <template #footer>
