<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="คำแนะนำแก้ปัญหาระบบสืบพันธุ์" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">คำแนะนำแก้ปัญหาระบบสืบพันธุ์</h1>
          </div>
        ...
        >
          ...
          <Column
            field="ReproduceSuggestionName"
            header="ชื่อคำแนะนำแก้ปัญหาระบบสืบพันธุ์"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.ReproduceSuggestionID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.ReproduceSuggestionID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อคำแนะนำแก้ปัญหาระบบสืบพันธุ์
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.ReproduceSuggestionName"
                :class="{ 'p-invalid': !form.ReproduceSuggestionName && valid }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/reproduce-suggestion",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "คำแนะนำแก้ปัญหาระบบสืบพันธุ์", to: "" },
      ],
      search: {
      ...
      valid: false,
      form: {
        ReproduceSuggestionCode: "",
        ReproduceSuggestionName: "",
        ReproduceSuggestionNameEN: "",
        isActive: "",
      },
  ...
  methods: {
    ...
    validation() {
      if (!this.form.ReproduceSuggestionName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.ReproduceSuggestionID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.ReproduceSuggestionID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        ReproduceSuggestionID: id.ReproduceSuggestionID,
        ReproduceSuggestionCode: id.ReproduceSuggestionCode,
        ReproduceSuggestionName: id.ReproduceSuggestionName,
        ReproduceSuggestionNameEN: id.ReproduceSuggestionNameEN,
        show_id: id.show_id,
