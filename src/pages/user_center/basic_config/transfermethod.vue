<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="วิธีการย้ายฝาก" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">วิธีการย้ายฝาก</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="TransferMethodCode"
            header="รหัสวิธีการย้ายฝาก"
            class="text-center"
            :sortable="true"
          >
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.TransferMethodID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.TransferMethodID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสวิธีการย้ายฝาก</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.TransferMethodCode && valid }"
                v-model="form.TransferMethodCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.TransferMethodName"
                :class="{ 'p-invalid': !form.TransferMethodName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.TransferMethodCode || !this.form.TransferMethodName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.TransferMethodID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.TransferMethodID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        TransferMethodID: id.TransferMethodID,
        TransferMethodCode: id.TransferMethodCode,
        TransferMethodName: id.TransferMethodName,
        TransferMethodNameEN: id.TransferMethodNameEN,
        show_id: id.show_id,
