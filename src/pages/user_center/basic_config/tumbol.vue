<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ตำบล" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ตำบล</h1>
          </div>
        ...
        >
          ...
          <Column
            field="TumbolName"
            header="ชื่อตำบล (ภาษาไทย)"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="TumbolNameEN"
            header="ชื่อตำบล (ภาษาอังกฤษ)"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.TumbolID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(slotProps.data.TumbolID, slotProps.data.show_id)
                "
              />
            </template>
          </Column>
        ...
        <div class="col-12 lg:col-12">
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">รหัสตำบล</label>
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.TumbolCode && valid }"
                v-model="form.TumbolCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.TumbolName"
                :class="{ 'p-invalid': !form.TumbolName && valid }"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.TumbolNameEN"
                :class="{ 'p-invalid': !form.TumbolNameEN && valid }"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <Dropdown
                ...
                :class="{ 'p-invalid': !form.ProvinceID && valid }"
                :options="province"
                :filter="true"
              />
            </div>
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">อำเภอ</label>
              <Dropdown
                class="w-full"
                placeholder="เลือกอำเภอ"
                optionLabel="Fullname"
                optionValue="AmphurID"
                v-model="form.AmphurID"
                :class="{ 'p-invalid': !form.AmphurID && valid }"
                :options="amphur"
                :filter="true"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"> รหัสไปรษณีย์ </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.Zipcode"
                :class="{ 'p-invalid': !form.Zipcode && valid }"
                maxlength="5"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (
        !this.form.TumbolCode ||
        !this.form.TumbolName ||
        !this.form.AmphurID ||
        !this.form.Zipcode ||
    ...
    load() {
      ...
      // ดึงข้อมูลจังหวัด
      axios.get(this.getProvince).then((response) => {
        this.province = response.data.rows;
      });

      // ดึงข้อมูลอำเภอ
      axios.get(this.getAmphur).then((response) => {
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.TumbolID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.TumbolID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        TumbolID: id.TumbolID,
        TumbolCode: id.TumbolCode,
        TumbolName: id.TumbolName,
        TumbolNameEN: id.TumbolNameEN,
        AmphurID: id.AmphurID,
