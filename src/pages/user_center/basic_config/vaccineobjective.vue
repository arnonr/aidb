<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="วัตถุประสงค์การฉีดวัคซีน" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">วัตถุประสงค์การฉีดวัคซีน</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="VaccineObjectiveCode"
            header="รหัสวัตถุประสงค์การฉีดวัคซีน"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="VaccineObjectiveName"
            header="ชื่อวัตถุประสงค์การฉีดวัคซีน"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.VaccineObjectiveID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.VaccineObjectiveID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสวัตถุประสงค์การฉีดวัคซีน</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.VaccineObjectiveCode && valid }"
                v-model="form.VaccineObjectiveCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อวัตถุประสงค์การฉีดวัคซีน
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.VaccineObjectiveName"
                :class="{ 'p-invalid': !form.VaccineObjectiveName && valid }"
              />
            </div>
...
export default {
  ...
  data() {
    return {
      url: "/vaccine-objective",
      breadcrumb: [
        { label: "ข้อมูลพื้นฐานระบบ", to: "/user_center/dashboard" },
        { label: "วัตถุประสงค์การฉีดวัคซีน", to: "" },
      ],
      search: {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.VaccineObjectiveCode || !this.form.VaccineObjectiveName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.VaccineObjectiveID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.VaccineObjectiveID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        VaccineObjectiveID: id.VaccineObjectiveID,
        VaccineObjectiveCode: id.VaccineObjectiveCode,
        VaccineObjectiveName: id.VaccineObjectiveName,
        VaccineObjectiveNameEN: id.VaccineObjectiveNameEN,
        show_id: id.show_id,
