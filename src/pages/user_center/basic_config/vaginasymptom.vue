<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="ปัญหามดลูก/ช่องคลอด" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">ปัญหามดลูก/ช่องคลอด</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="VaginaSymptomCode"
            header="รหัสปัญหามดลูก/ช่องคลอด"
            class="text-center"
            :sortable="true"
          >
          </Column>
          <Column
            field="VaginaSymptomName"
            header="ชื่อปัญหามดลูก/ช่องคลอด"
            class="text-center"
            :sortable="true"
          >
          </Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.VaginaSymptomID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.VaginaSymptomID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสปัญหามดลูก/ช่องคลอด</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.VaginaSymptomCode && valid }"
                v-model="form.VaginaSymptomCode"
              />
            </div>

            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2">
                ชื่อปัญหามดลูก/ช่องคลอด
              </label>
              <InputText
                type="text"
                class="w-full"
                v-model="form.VaginaSymptomName"
                :class="{ 'p-invalid': !form.VaginaSymptomName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.VaginaSymptomCode || !this.form.VaginaSymptomName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.VaginaSymptomID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.VaginaSymptomID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        VaginaSymptomID: id.VaginaSymptomID,
        VaginaSymptomCode: id.VaginaSymptomCode,
        VaginaSymptomName: id.VaginaSymptomName,
        VaginaSymptomNameEN: id.VaginaSymptomNameEN,
        show_id: id.show_id,
