<template>
  <div class="grid">
    <!-- start main page -->
    <div class="col-12">
      <PageTitle title="เขตุปศุสัตว์" :pages="breadcrumb" />
      ...
      <div class="mt-5 card">
        <div class="grid flex align-items-center mb-5">
          <div class="col-12 md:col-6">
            <h1 class="text-2xl mb-0 text-600">เขตุปศุสัตว์</h1>
          </div>
        ...
        >
          ...
          ></Column>
          <Column
            field="OrganizationZoneCode"
            header="รหัสเขตุปศุสัตว์"
            class="text-center"
            :sortable="true"
          ></Column>
          <Column
            field="OrganizationZoneName"
            header="ชื่อเขตุปศุสัตว์"
            class="text-center"
            :sortable="true"
          ></Column>
          ...
          <Column header="จัดการ" class="text-center">
            <template #body="slotProps">
              <Button
                ...
                @click="open_edit(slotProps.data.OrganizationZoneID)"
              />
              <Button
                label="ลบ"
                icon="pi pi-trash"
                ...
                @click="
                  open_delete(
                    slotProps.data.OrganizationZoneID,
          ...
          <div class="formgrid grid">
            <div class="field col-12 sm:col-12">
              <label class="block text-sm font-bold mb-2"
                >รหัสเขตุปศุสัตว์</label
              >
              <InputText
                type="text"
                class="w-full"
                :class="{ 'p-invalid': !form.OrganizationZoneCode && valid }"
                v-model="form.OrganizationZoneCode"
              />
            </div>
            ...
            <div class="field col-12 sm:col-12">
              ...
              <InputText
                type="text"
                class="w-full"
                v-model="form.OrganizationZoneName"
                :class="{ 'p-invalid': !form.OrganizationZoneName && valid }"
              />
            </div>
...
export default {
  ...
  methods: {
    ...
    validation() {
      if (!this.form.OrganizationZoneCode || !this.form.OrganizationZoneName) {
        this.valid = true;
        this.$toast.add({
          severity: "error",
          summary: "ล้มเหลว",
    ...
    update() {
      ...
      this.form.isActive = this.form.isActive.value;
      axios
        .put(this.url + "/" + this.form.OrganizationZoneID, this.form)
        .then(() => {
          this.close_edit();
          this.load();
          this.$toast.add({
    ...
    open_edit(id) {
      id = this.data.find((x) => x.OrganizationZoneID == id);
      if (id.isActive == 0) {
      ...
      this.form = {
        ...
        OrganizationZoneCode: id.OrganizationZoneCode,
        OrganizationZoneName: id.OrganizationZoneName,
        OrganizationZoneNameEN: id.OrganizationZoneNameEN,
        show_id: id.show_id,
